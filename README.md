# Shaders used in ConflictSolver (PART 1) 

*NOTE: FOR FINAL PROJECT, PLEASE SCROLL DOWN TO "Final Project Effects and Shaders Implemented" section*
...
Scrolling Numbers effect/Scrolling UV shader:
Furthermore with the barrier, it also has a dynamic visual effect where on the surfaces there is binary numbers scrolling by, and this was accomplished by sampling the same texture twice, and the texture we used was this image of a glowing 0 and 1 which was made in GIMP:

<img width="62" height="232" alt="image" src="https://github.com/user-attachments/assets/d7327849-ecf9-4559-a23f-92a13deded67" />

<img width="1407" height="813" alt="image" src="https://github.com/user-attachments/assets/df1b7226-20eb-48da-b98f-605e7fd8aca4" />

From there, for each of the two sample texture nodes it's UVs are modified by the Tiling and Offset node, which uses vector 2 properties to control the tiling amount on the X and Y axises which dictates how many binary numebrs are supposed to appear on the object surfaces, and another vector 2 property was used to control how it should tile on the x and y axis. There is also the time node being multiplied with a scalar value property which controls the speed that gets multplied with the offset along with the scolling 2D vector property which animates the texture to the scroll. The textures being sampled are also set to be repeating so there is no weird stretching, and the reason two of the samplers sample the same texture was so that there can be an overlap of scrolling binary numbers for extra depth to make it look like the barrier has numbers inside of it. The binary numbers at the front scroll faster than the ones at the back as their speed is cut in half.

<img width="648" height="432" alt="image" src="https://github.com/user-attachments/assets/02f69e10-45b2-4972-bf93-41fad3309bb1" />

We used this effect for our game to further emphasize that the instability of the glitchy barrier, where the binary numbers which are associated with programming and bits are scrolling by to make the barrier look more dynamic in the game world instead of being static. Unlike the other environment set pieces where the closest thing to "errors" for the green glowing platformers are error messages, this along with the hologram shader entirely exagerrates the how broken and out of place this platform is in the world and further emphasizing its vulnurability in both being a massive programming error but also being destructable as well. 
...
Emission Effect:
Another thing about the wires is the use of emission, specifically the glowing green sections of the wire emitting light. This was accomplished by sampling the emission texture for the wire, multiply it with the calculations for the dot product rim lighting with using the view direction and object normals in world space. However, this time the lighting is not restricted within the power node to covert the entire texture and it is multipied with a property to increase the brightness. Afterwards that gets added on top of the base texture for the final color output for the wires. 
...
Liquid effects:
This shader makes mesh looks "liquidy." The waves are generated by vertex displacement using a noise and modifying the offset using the time node to animate it and give it movements. This is also used to animate the foam and edge foam. These are also customizable as well: wave height will determines how far the vertices can at away from the center, speed multiplies with the time node to adjust for fast or slow the waves moves, there is also base texture, color, as welll as it being applies to the foam and edge foam too. The time nodes have different slots, and these are picked based on how we wanted the behaviour of the liquid to be: time for constant movement, and 
sine/cosine for “back and forth” movement (sway).

<img width="1537" height="942" alt="water shadeer" src="https://github.com/user-attachments/assets/97d11bf9-ffe9-4239-b019-8b21833f4524" />

This shader are used in the Push/Pull boss room, and BitHub boss room. This is specifiacally placed there as both BitHub and Push/Pull are based on aquatic monsters (crocodile/kraken hybrid, and the hydra respectively).

Push/Pull room before:

<img width="2048" height="1454" alt="image" src="https://github.com/user-attachments/assets/75176357-3acc-42e0-81ad-0611e61c97a1" />
...
<img width="1488" height="744" alt="image" src="https://github.com/user-attachments/assets/e067597a-3c2e-486d-9d96-d58d0278ae52" />
